...
valid_obj = Input_neg(valid, hist_valid, neg_samples, N, n_neg, do_shuffle=False)
...
prob_valid, logits_valid = make_decoder(z, x_observed, hs, x_dim, c1_dim, c2_dim, h_decode, z_dim, seed=random_seed, scope='decoder')
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
    ...
    for step in range(valid_obj.epoch_size):
        valid_full, valid_observed, valid_unobserved, valid_hs, samples = valid_obj.next_batch()
        prob_list = []
        for _ in range(n_samples):
            prob = sess.run(prob_valid, feed_dict={x_observed: valid_observed, z:np.random.normal(size=(1,z_dim)), hs: valid_hs})
    ...
    with open('./results/hit_z%s_h%s_t%s_lr%s_valid.txt' % (zd, hd, ts, lr), 'w') as file:
        for topk in topk_list:

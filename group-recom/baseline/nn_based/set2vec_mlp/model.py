...
class Input(object):
    def __init__(self, data, length, N, batch_size, max_len, shuffle_row=False, shuffle_col=False):
        self.data = data
        self.length = length
        self.N = N
    ...
    def next_batch(self):
        ...
        mask_x = np.take(self.diag_ones, x_len-1, axis=0)

        #mask_y = np.take(self.lower_ones, x_len-2, axis=0)
...
class Input_neg(object):
    def __init__(self, data, length, neg_samples, N, max_len, n_neg=200, shuffle_row=False, shuffle_col=False):
        self.data = data
        self.length = length
    ...
    def next_batch(self):
        ...
        samples[self.n_neg, x[0][:x_len[0]-1]] = 1.0

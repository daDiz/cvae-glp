...
print_iter = 20
#valid_iter = 10 # calc hit rate on the validation set
                # for every valid_iter epochs
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
    ...
    with open('./results/hit_at%s_ed%s_hd%s_lr%s_ts%s_test.txt' % (top, ed, hd, lr, ts), 'w') as file:
        file.write('%f\n' % (np.mean(hit_rate)))
